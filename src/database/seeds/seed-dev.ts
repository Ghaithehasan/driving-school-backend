...
const PERMISSIONS: { code: string; module: string; description: string }[] = [
  ...
  {
    code: 'instructors.create',
    module: 'instructors',
    description: 'إنشاء مدرس',
  },
  {
    code: 'instructors.read',
    module: 'instructors',
    description: 'عرض المدرسين',
  },
  {
    code: 'instructors.update',
    module: 'instructors',
    description: 'تعديل مدرس',
  },
  ...
  { code: 'vehicles.delete', module: 'vehicles', description: 'حذف سيارة' },
  {
    code: 'vehicles.fuel',
    module: 'vehicles',
    description: 'تعبئة وقود سيارة',
  },
  {
    code: 'vehicles.maintenance',
    module: 'vehicles',
    description: 'إدارة صيانة السيارة',
  },
  ...
  {
    code: 'bookings.cancel-own',
    module: 'bookings',
    description: 'إلغاء الطالب لحجزه',
  },
  ...
  },
  {
    ...
    module: 'payments',
    description: 'تحصيل باقي المبلغ وإتمام الدرس',
  },

  {
    code: 'certificates.create',
    module: 'certificates',
    description: 'إنشاء شهادة',
  },
  {
    code: 'certificates.read',
    module: 'certificates',
    description: 'عرض الشهادات',
  },
  {
    code: 'certificates.update',
    module: 'certificates',
    description: 'تعديل شهادة',
  },
  {
    code: 'certificates.cancel',
    module: 'certificates',
    description: 'إلغاء شهادة',
  },
...
const ROLE_PERMISSIONS: Record<RoleTitle, string[]> = {
  ...
  [RoleTitle.INSTRUCTOR]: ['bookings.read', 'notifications.read'],
...
const VEHICLES: {
  ...
  // ── أوتوماتيك نشطة ──
  {
    plateNumber: 'أ ب ج 201',
    model: 'هيونداي إلنترا 2021',
    color: 'أسود',
    type: VehicleType.AUTOMATIC,
    status: VehicleStatus.ACTIVE,
  },
  {
    plateNumber: 'أ ب ج 202',
    model: 'هيونداي إلنترا 2022',
    color: 'أبيض',
    type: VehicleType.AUTOMATIC,
    status: VehicleStatus.ACTIVE,
  },
  ...
  // ── عادي نشطة ──
  {
    plateNumber: 'أ ب ج 101',
    model: 'تويوتا كورولا 2020',
    color: 'أبيض',
  ...
  {
    plateNumber: 'أ ب ج 203',
    model: 'هيونداي إلنترا 2020',
    color: 'أحمر',
    type: VehicleType.AUTOMATIC,
  ...
  {
    plateNumber: 'أ ب ج 102',
    model: 'تويوتا كورولا 2019',
    color: 'رمادي',
    type: VehicleType.MANUAL,
    status: VehicleStatus.INACTIVE,
    adminNotes: 'تغيير زيت',
    maintenance: true,
  },

  // ── مؤرشفة (2 أوتو + 2 عادي) ──
  {
    plateNumber: 'أ ب ج 204',
    model: 'هيونداي إلنترا 2018',
    color: 'أبيض',
    type: VehicleType.AUTOMATIC,
    status: VehicleStatus.ARCHIVED,
  },
  {
    plateNumber: 'أ ب ج 205',
    model: 'هيونداي إلنترا 2019',
    color: 'رمادي',
    type: VehicleType.AUTOMATIC,
    status: VehicleStatus.ARCHIVED,
  },
  {
    plateNumber: 'أ ب ج 104',
    model: 'تويوتا كورولا 2017',
    color: 'أبيض',
    type: VehicleType.MANUAL,
    status: VehicleStatus.ARCHIVED,
  },
  {
    plateNumber: 'أ ب ج 105',
...
const INSTRUCTOR_USERS = [
  ...
  },
  {
    ...
    phone: '0999300002',
    gender: Gender.MALE,
    instructorType: InstructorType.AUTOMATIC,
  },
  {
    name: 'فاطمة محمد',
    phone: '0999300003',
    gender: Gender.FEMALE,
    instructorType: InstructorType.BOTH,
  },
  {
    name: 'نور حسين',
    phone: '0999300004',
...
// ─── دوال مساعدة إضافية ─────────────────────────────────────────────────────
async function trainingSessionExists(
  repo: any,
  certId: number,
): Promise<boolean> {
  ...
  await AppDataSource.transaction(async (manager) => {
    ...
    for (const [roleTitle, codes] of Object.entries(ROLE_PERMISSIONS)) {
      const role = await roleRepo.findOne({
        where: { title: roleTitle as RoleTitle },
      });
      if (!role) {
        console.log(
          `  ⚠️  الدور ${roleTitle} غير موجود — شغّل seed:admin أولاً`,
        );
        continue;
      }
      for (const code of codes) {
        ...
        if (count === 0) {
          await rolePermRepo.save(
            rolePermRepo.create({ role, permission: perm }),
          );
    ...
    for (const data of EMPLOYEE_USERS) {
      ...
      if (!user) {
        console.log(`  ↩️  موجود مسبقاً: ${data.phone}`);
        continue;
      }
      const role = await roleRepo.findOneOrFail({
        where: { title: data.role },
      });
    ...
    // 4) المدرسون
    console.log('\n── إنشاء المدرسين ──');
    const instructorRole = await roleRepo.findOneOrFail({
    ...
    for (const data of INSTRUCTOR_USERS) {
      ...
      console.log(
        `  ✅ ${data.name} (${data.gender} — ${data.instructorType})`,
      );
    }

    // 5) الطلاب
    console.log('\n── إنشاء الطلاب ──');
    const studentRole = await roleRepo.findOneOrFail({
    ...
    for (const data of VEHICLES) {
      ...
      if (data.maintenance) {
        ...
        await unavailableRepo.save(
          unavailableRepo.create({
            vehicle,
            startAt,
            endAt: null,
    ...
    const activeVehicles = savedVehicles.filter(
    ...
    const vehicleExpenses: {
      ...
      {
        vehicleIdx: 1,
        reason: VehicleExpenseReason.INSURANCE,
        liters: null,
        amount: '350.00',
        expenseDate: '2026-01-05',
        status: ExpenseStatus.PAID,
        note: 'تجديد تأمين سنوي',
      },
      {
        vehicleIdx: 2,
        reason: VehicleExpenseReason.INSURANCE,
      ...
      },
      {
        ...
        status: ExpenseStatus.PAID,
        note: 'تغيير زيت وفلتر',
      },
      {
        vehicleIdx: 0,
        reason: VehicleExpenseReason.OTHER,
        liters: null,
        amount: '80.00',
    ...
    for (const row of vehicleExpenses) {
      ...
      await expVehicleRepo.save(
        expVehicleRepo.create({
          reason: row.reason,
          liters: row.liters,
          vehicle,
          expense,
    ...
    const students = await manager
      ...
      .find({ relations: { user: true }, take: 10 });
    const instructors = await manager
      .getRepository(Instructor)
      .find({ take: 4 });
    const activeVehiclesList = savedVehicles.filter(
      (v) => v.status === VehicleStatus.ACTIVE,
    );
    ...
    if (
      ...
      const rows: BookingRow[] = [
        // ── مكتملة ──
        {
          ...
          source: VehicleSource.SCHOOL_CAR,
          bStatus: BookingStatus.COMPLETED,
          pStatus: PaymentStatus.FULLY_PAID,
          date: '2026-05-08',
          hour: 9,
        },
        {
        ...
        // ── لم يحضر ──
        {
          ...
          trainingType: TrainingType.MANUAL,
          source: VehicleSource.SCHOOL_CAR,
          bStatus: BookingStatus.NO_SHOW,
          pStatus: PaymentStatus.DEPOSIT_PAID,
          date: '2026-06-10',
          hour: 9,
        },
        {
        ...
        // ── منتهي الصلاحية ──
        {
          ...
          trainingType: TrainingType.MANUAL,
          source: VehicleSource.SCHOOL_CAR,
          bStatus: BookingStatus.EXPIRED,
          pStatus: PaymentStatus.DEPOSIT_NON_REFUNDABLE,
          date: '2026-06-15',
          hour: 9,
        },
        {
        ...
        // الطالب يُلغي → العربون غير مسترد (NON_REFUNDABLE)
        {
          ...
          date: '2026-06-08',
          hour: 9,
          cancelParty: CancellationParty.STUDENT,
        ...
        // المدرسة تُلغي → العربون متاح للنقل لحجز جديد (AVAILABLE_FOR_REBOOKING)
        {
          ...
          vehicleIdx: null,
          trainingType: TrainingType.MANUAL,
          source: VehicleSource.STUDENT_CAR,
          bStatus: BookingStatus.CANCELLED,
          pStatus: PaymentStatus.DEPOSIT_AVAILABLE_FOR_REBOOKING,
          date: '2026-06-16',
          hour: 10,
          cancelParty: CancellationParty.SCHOOL,
          cancelReason: 'غياب المدرب',
        },
        ...
        // ── بانتظار الدفع (مدرس مختلف + سيارة مختلفة لكل صف) ──
        {
          ...
          trainingType: TrainingType.MANUAL,
          source: VehicleSource.SCHOOL_CAR,
          bStatus: BookingStatus.PENDING_PAYMENT,
          pStatus: PaymentStatus.PENDING_DEPOSIT,
          date: '2026-06-28',
          hour: 9,
        },
        {
      ...
      for (const row of rows) {
        ...
        const vehicle =
          row.vehicleIdx !== null
            ? activeVehiclesList[row.vehicleIdx % activeVehiclesList.length]
            : null;
      ...
      const lessonPrices: {
        ...
        {
          gender: Gender.MALE,
          training: TrainingType.AUTOMATIC,
          source: VehicleSource.SCHOOL_CAR,
          price: '3000.00',
        },
        {
      ...
      for (const lp of lessonPrices) {
        const exists = await lessonPriceRepo.findOne({
          where: {
            instructorGender: lp.gender,
            trainingType: lp.training,
            vehicleSource: lp.source,
            effectiveFrom: '2026-01-01',
          },
        });
        if (!exists) {
          await lessonPriceRepo.save(
            lessonPriceRepo.create({
              instructorGender: lp.gender,
              trainingType: lp.training,
              vehicleSource: lp.source,
              price: lp.price,
      ...
      console.log(
        `  ✅ ${lpCount} سعر درس (${lessonPrices.length - lpCount} موجود مسبقاً)`,
      );
      ...
      for (const [type, price] of [
        [InstructorPriceType.MALE, '500.00'],
        [InstructorPriceType.FEMALE, '600.00'],
      ] as const) {
        const exists = await instructorPriceRepo.findOne({
          where: { type, effectiveFrom: '2026-01-01' },
        });
        if (!exists)
          await instructorPriceRepo.save(
            instructorPriceRepo.create({
              type,
              price,
              effectiveFrom: '2026-01-01',
            }),
      ...
      const SETTINGS = [
        ...
        },
        {
          ...
          value: '0111234567',
          valueType: SettingValueType.STRING,
          description: 'هاتف المدرسة',
        },
        {
          key: 'lesson_duration_minutes',
          value: '60',
          valueType: SettingValueType.NUMBER,
          description: 'مدة الحصة بالدقائق',
        },
        {
          key: 'deposit_amount',
          value: '500',
          valueType: SettingValueType.NUMBER,
          description: 'مبلغ العربون الثابت (legacy)',
        },
        {
          key: 'deposit_percentage',
          value: '50',
          valueType: SettingValueType.PERCENT,
          description: 'نسبة العربون من سعر الدرس',
        },
        {
          key: 'booking_window_days',
          value: '4',
          valueType: SettingValueType.NUMBER,
          description: 'عدد أيام نافذة الحجز المتاحة',
        },
        {
        ...
        },
        {
          ...
          value: 'غيث ابراهيم حسن',
          valueType: SettingValueType.STRING,
          description: 'اسم مستلم شام كاش المعتمد للتحقق من دفعات الطلاب',
        },
        {
          key: 'max_bookings_per_day',
          value: '8',
          valueType: SettingValueType.NUMBER,
          description: 'أقصى حجوزات يومية للمدرب',
        },
        {
          key: 'booking_lock_minutes',
          value: '30',
          valueType: SettingValueType.NUMBER,
          description: 'مدة قفل الحجز (legacy)',
        },
        {
          key: 'cancellation_fee_percent',
          value: '0',
          valueType: SettingValueType.PERCENT,
          description: 'نسبة رسوم الإلغاء',
        },
        {
        ...
        {
          key: 'allow_student_car',
          value: 'true',
          valueType: SettingValueType.BOOLEAN,
          description: 'السماح بسيارة الطالب',
        },
      ];
      let settingCount = 0;
      for (const s of SETTINGS) {
        ...
        if (!exists) {
          await settingRepo.save(
            settingRepo.create({ ...s, updatedAt: new Date() }),
          );
          settingCount++;
        }
      }
      ...
      const allEmployees = await manager
      ...
      for (const emp of allEmployees) {
        for (const month of salaryMonths) {
          const existsEmp = await expEmpRepo.findOne({
            where: { employee: { id: emp.id }, month },
          });
          if (!existsEmp) {
            const expense = await expRepo.save(
              expRepo.create({
                category: ExpenseCategory.EMPLOYEE,
                amount: '150000.00',
                expenseDate: month,
                status: ExpenseStatus.PAID,
                note: `راتب شهر ${month.slice(0, 7)}`,
                employee: emp,
              }),
            );
            await expEmpRepo.save(
              expEmpRepo.create({
                type: EmployeeExpenseType.SALARY,
                month,
                employee: emp,
                expense,
              }),
      ...
      if (allEmployees[0]) {
        ...
        const existsBonus = await expEmpRepo.findOne({
          where: {
            employee: { id: allEmployees[0].id },
            month: bonusMonth,
            type: EmployeeExpenseType.BONUS,
          },
        });
        if (!existsBonus) {
      ...
      const generalItems: {
        ...
        {
          type: GeneralExpenseType.ELECTRICITY,
          amount: '45000.00',
          date: '2026-04-01',
          note: 'فاتورة كهرباء أبريل',
        },
        {
          type: GeneralExpenseType.ELECTRICITY,
          amount: '42000.00',
          date: '2026-05-01',
          note: 'فاتورة كهرباء مايو',
        },
        {
          type: GeneralExpenseType.ELECTRICITY,
          amount: '48000.00',
        ...
        {
          type: GeneralExpenseType.INTERNET,
          amount: '12000.00',
          date: '2026-04-01',
          note: 'اشتراك إنترنت',
        },
        ...
        {
          type: GeneralExpenseType.KITCHEN,
          amount: '15000.00',
          date: '2026-05-15',
          note: 'مستلزمات المطبخ',
        },
        {
          type: GeneralExpenseType.SUPPLIES,
          amount: '20000.00',
          date: '2026-06-10',
          note: 'قرطاسية ومستلزمات مكتبية',
        },
      ];
      for (const g of generalItems) {
        ...
        await expGenRepo.save(
          expGenRepo.create({ type: g.type, expense: exp }),
        );
      }
      ...
      for (const booking of completedBookings) {
        const existsInst = await expInstRepo.findOne({
        ...
        if (!existsInst) {
          const exp = await expRepo.save(
            expRepo.create({
              ...
              amount: '500.00',
              expenseDate: booking.startAt.toISOString().split('T')[0],
              status: ExpenseStatus.PAID,
              note: null,
              employee: null,
            }),
      ...
      for (const booking of savedBookings) {
        const existsCharge = await chargeRepo.findOne({
          where: {
        ...
        if (booking.bookingStatus === BookingStatus.COMPLETED) {
          const charge = await chargeRepo.save(
            chargeRepo.create({
              chargeReason: ChargeReason.LESSON,
              amountDue: '2500.00',
          ...
          await paymentRepo.save(
            paymentRepo.create({
              ...
              receivedAt: new Date(booking.startAt.getTime() - 86400000 * 3),
          ...
          await paymentRepo.save(
            paymentRepo.create({
              amountPaid: '2000.00',
              paymentMethod: PaymentMethod.CASH,
              receivedAt: booking.startAt,
              studentCharge: charge,
            }),
          );
          chargeCount++;
        } else if (
          [BookingStatus.BOOKED, BookingStatus.NO_SHOW].includes(
          ...
          await paymentRepo.save(
            paymentRepo.create({
              amountPaid: '500.00',
              paymentMethod: PaymentMethod.CASH,
              receivedAt: new Date(booking.startAt.getTime() - 86400000 * 2),
              studentCharge: charge,
            }),
          );
          chargeCount++;
          ...
          await chargeRepo.save(
            chargeRepo.create({
              ...
              certificate: null,
              certificateExamResult: null,
              dueAt: new Date(booking.startAt.getTime() - 86400000)
                .toISOString()
                .split('T')[0],
            }),
          );
          ...
          await paymentRepo.save(
            paymentRepo.create({
              amountPaid: '500.00',
              paymentMethod: PaymentMethod.CASH,
              receivedAt: new Date(booking.startAt.getTime() - 86400000 * 5),
              studentCharge: charge,
            }),
          );
          chargeCount++;
      ...
      const trainingSessRepo = manager.getRepository(
        CertificateTrainingSession,
      );
      ...
      const certData: {
        ...
        {
          studentIdx: 0,
          category: CertificateCategory.B,
        ...
        },
        {
          studentIdx: 1,
          category: CertificateCategory.B,
          status: CertificateRequestStatus.WAITING_FOR_PRACTICAL_EXAM,
          transport: true,
        ...
        },
        {
          studentIdx: 2,
          category: CertificateCategory.B,
          status: CertificateRequestStatus.IN_GOVERNMENT_TRAINING,
          transport: true,
          requestedAt: new Date('2026-05-01'),
        },
        {
          studentIdx: 3,
          category: CertificateCategory.B1,
          status: CertificateRequestStatus.WAITING_FOR_TRAINING_SCHEDULE,
          transport: false,
        ...
        },
        {
          studentIdx: 4,
          category: CertificateCategory.B,
          status: CertificateRequestStatus.WAITING_FOR_SERVICE_FEE,
          transport: false,
      ...
      for (const cd of certData) {
        ...
        const existing = await certRepo.findOne({
          where: { student: { id: student.id }, category: cd.category },
        });
        if (existing) {
          savedCerts.push(existing);
          continue;
        }
        const cert = await certRepo.save(
          certRepo.create({
            category: cd.category,
            requestStatus: cd.status,
            personalPhotoUrl: `https://example.com/photos/student_${student.id}.jpg`,
            transportRequested: cd.transport,
            requestedAt: cd.requestedAt,
            student,
          }),
        );
      ...
      if (savedCerts[0]) {
        const existsExam = await examResultRepo.findOne({
          where: {
            certificate: { id: savedCerts[0].id },
        ...
        if (!existsExam) {
          await examResultRepo.save(
            examResultRepo.create({
              certificate: savedCerts[0],
              examType: ExamType.THEORY,
              attemptNumber: 1,
              scheduledAt: new Date('2026-03-15'),
              examResult: ExamResult.PASS,
              resultRecordedAt: new Date('2026-03-15'),
            }),
          );
          await examResultRepo.save(
            examResultRepo.create({
              ...
              attemptNumber: 1,
              scheduledAt: new Date('2026-03-25'),
              examResult: ExamResult.FAIL,
              resultRecordedAt: new Date('2026-03-25'),
            }),
          );
          await examResultRepo.save(
            examResultRepo.create({
              ...
              examResult: ExamResult.PASS,
              resultRecordedAt: new Date('2026-04-05'),
            }),
          );
        }
      }
      ...
      if (savedCerts[1]) {
        const existsExam = await examResultRepo.findOne({
          where: {
            certificate: { id: savedCerts[1].id },
            examType: ExamType.THEORY,
          },
        });
        if (!existsExam) {
          await examResultRepo.save(
            examResultRepo.create({
              certificate: savedCerts[1],
              examType: ExamType.THEORY,
              attemptNumber: 1,
              scheduledAt: new Date('2026-05-10'),
              examResult: ExamResult.PASS,
              resultRecordedAt: new Date('2026-05-10'),
            }),
          );
          await examResultRepo.save(
            examResultRepo.create({
      ...
      if (savedCerts[2]) {
        const existsSess = await trainingSessionExists(
          trainingSessRepo,
          savedCerts[2].id,
        );
        if (!existsSess) {
          for (let i = 1; i <= 6; i++) {
            const scheduledAt =
              i <= 4 ? new Date(`2026-05-${10 + i * 2}`) : null;
            await trainingSessRepo.save(
              trainingSessRepo.create({
                certificate: savedCerts[2],
                sessionNumber: i,
                scheduledAt,
              }),
            );
      ...
      console.log(
        `  ✅ ${savedCerts.length} شهادة + نتائج امتحانات + جلسات تدريب`,
      );
      ...
      const trips = [
        {
          tripType: TripType.LECTURE,
          tripDate: '2026-05-20',
          dayNumber: 1,
          assemblyTime: '07:30:00',
          destination: 'مركز التدريب الحكومي',
          capacity: 20,
        ...
        {
          tripType: TripType.EXAM,
          tripDate: '2026-06-15',
          dayNumber: null,
          assemblyTime: '08:00:00',
          destination: 'مديرية السير',
          capacity: 15,
          status: TripStatus.COMPLETED,
        },
        {
      ...
      for (const t of trips) {
        const existing = await tripRepo.findOne({
          where: { tripDate: t.tripDate, tripType: t.tripType },
        });
        if (existing) {
        ...
        const trip = await tripRepo.save(
          tripRepo.create({ ...t, createdBy: receptionist }),
        );
      ...
      for (const cert of certsWithTransport) {
        for (const trip of savedTrips.filter(
          (t) => t.status === TripStatus.COMPLETED,
        )) {
          const existsReg = await regRepo.findOne({
            where: { certificate: { id: cert.id }, tripType: trip.tripType },
          });
          if (existsReg) continue;

          // رسوم النقل أولاً
          const chargeForTransport = await chargeRepo.save(
            chargeRepo.create({
              chargeReason:
                trip.tripType === TripType.LECTURE
                  ? ChargeReason.TRANSPORT_LECTURE
                  : ChargeReason.TRANSPORT_EXAM,
              amountDue: '5000.00',
              chargeStatus: ChargeStatus.PAID,
          ...
          await paymentRepo.save(
            paymentRepo.create({
              amountPaid: '5000.00',
              paymentMethod: PaymentMethod.CASH,
              receivedAt: new Date(trip.tripDate),
              studentCharge: chargeForTransport,
            }),
          );

          const reg = await regRepo.save(
            regRepo.create({
          ...
          await attnRepo.save(
            attnRepo.create({
              transportRegistration: reg,
              transportTrip: trip,
              attendanceStatus: AttendanceStatus.ATTENDED,
              attendedAt: new Date(trip.tripDate),
            }),
          );
        }
      }
      ...
      if (notifCount === 0) {
        const notifData = [
          {
            student: certStudents[0],
            title: 'تأكيد الحجز',
            body: 'تم تأكيد حجزك للدرس بنجاح.',
            type: NotificationType.BOOKING_CONFIRMED,
          },
          {
            student: certStudents[1],
            title: 'تم إلغاء الحجز',
            body: 'تم إلغاء درسك بسبب ظرف طارئ.',
            type: NotificationType.BOOKING_CANCELLED,
          },
          {
            student: certStudents[2],
            title: 'تحديث حالة الشهادة',
            body: 'تم قبول طلب شهادتك وهي قيد المعالجة.',
            type: NotificationType.CERTIFICATE_STATUS_CHANGED,
          },
          {
            student: certStudents[0],
            title: 'استلام الدفعة',
            body: 'تم استلام دفعتك بمبلغ 500 ل.س.',
            type: NotificationType.PAYMENT_ACCEPTED,
          },
          {
            student: certStudents[3],
            title: 'موعد الدرس غداً',
            body: 'تذكير: لديك درس غداً الساعة 9:00 صباحاً.',
